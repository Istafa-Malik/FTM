...
class RealTimeGraph(QMainWindow):
    ...
    def update_graph_data(self):
        if not self.is_paused:
            ...
            # Save data to CSV
            self.append_to_csv(elapsed_time, force)

            # Update graph
            self.update_graph()

    def append_to_csv(self, time_value, force_value):
        # Append data to the CSV file
        with open(self.csv_file, "a", newline="") as file:
            writer = csv.writer(file)
            writer.writerow([time_value, force_value])
    ...
    def show_saved_data_graph(self, time_data, force_data):
        ...
        dialog.setLayout(layout)
        dialog.resize(800, 600)
        dialog.show()  # Use `show()` instead of `exec_()` to open it as a non-blocking window

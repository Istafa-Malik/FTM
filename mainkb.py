import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QLineEdit, QVBoxLayout, QWidget, QPushButton, QGridLayout, QDialog, QHBoxLayout
...
class VirtualKeyboard(QDialog):
    def __init__(self, input_field, main_window):
        ...
        self.is_caps = True  # Flag to check if Caps Lock is on (start with uppercase)
        self.initUI()
...
class MainWindow(QMainWindow):
    ...
    def show_virtual_keyboard(self, event):
        if not self.keyboard_open:
            ...
            self.keyboard.finished.connect(self.on_keyboard_closed)  # Connect to track when the keyboard is closed
